...
async function resumen(req, res, next) {
    try {
        const [total, porEstado, interacciones, sinInteraccionReciente] = await Promise.all([
            totalClientes(),
            clientesPorEstado(),
            interaccionesPorCliente(),
            clientesSinInteraccionReciente()
        ]);

        res.json({ total, porEstado, interacciones, sinInteraccionReciente });
    } catch (err) {
        next(err);
    }
}
